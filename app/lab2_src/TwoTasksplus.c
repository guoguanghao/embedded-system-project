...
void task1(void* pdata)
{
  ...
  while(1)
  {
    ...
    for (i = 0; i < strlen(text2); i++)
        putchar(text2[i]);
    OSSemPend(task1Semaphore,0,&err_task1Semaphore);
	OSSemPost(task2Semaphore);
    //OSTimeDlyHMSM(0, 0, 1, 0); // Context Switch to next task
...
{
    INT8U err_task2Semaphore;
  while(1)
  { 
    char text2[] = "Hello from Task1\n";
    int i;
    for (i = 0; i < strlen(text2); i++)
        putchar(text2[i]);
    OSSemPend(task2Semaphore,0,&err_task2Semaphore);
